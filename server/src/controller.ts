...
export const chat = async (req: Request, res: Response) => {
  try {
    ...
    const searchData = await searchIndianKanoon(query);
...
export const viewPDF = async (req: Request, res: Response) => {
  try {
    ...
    const { data } = await axios.post(
      ...
      const metadata = await axios.post(`https://api.indiankanoon.org/docmeta/${tid}/`, {}, {
        headers: {
          Authorization: `Token ${process.env.INDIAN_KANOON_API_KEY}`,
          "Content-Type": "application/json",
        },
      });
      res.status(200).json({ data, metadata: metadata.data });
  } catch (error) {
